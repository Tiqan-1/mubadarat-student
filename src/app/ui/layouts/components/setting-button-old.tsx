...
import { presetsColors } from "@/framework/theme/tokens/color";
import { varHover } from "@/app/ui/components/animate/variants/action";
import { IconButton, Iconify, SvgIcon } from "@/app/ui/components/icon";

import { themeVars } from "@/framework/theme/theme.css";
import { FontFamilyPreset } from "@/framework/theme/tokens/typography";
import { type ThemeColorPresets, ThemeLayout, ThemeMode } from "@/framework/types/enum";
...
export default function SettingButton() {
	...
	const setDirection = (checked: boolean) => {
		setSettings({
			...settings,
			...
			<div className="flex items-center justify-center overflow-hidden">
				...
				>
					<IconButton className="h-10 w-10">
						<Iconify icon="solar:settings-broken" size="24" />
					</IconButton>
				</m.div>
			</div>
			<Drawer
					...
					<div>
						...
						<div className="flex flex-col gap-2">
							<div className="flex items-center justify-between text-sm text-text-disabled">
								<div>RTL</div>
								<Switch size="small" checked={direction === "rtl"} onChange={(checked) => setDirection(checked)} />
							</div>
							<div className="flex items-center justify-between text-sm text-text-disabled">
								<div>BreadCrumb</div>
